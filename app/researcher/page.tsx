'use client';

import { useState } from 'react';
import InstructionsSection from '../InstructionsSection';
import LoginOverlay from '../loginOverlay';
import VideoModal from '../VideoModal';
import SurveyInfoForm from '../SurveryInfoForm';
import SurveyDashboard from '../components/SurveyDashboard';
import { useAuth } from '../contexts/AuthContext';
import { useSurveyManager } from '../hooks/useSurveyManager';

export default function ResearcherPage() {
  const { isAuthenticated, loading } = useAuth();
  const { 
    surveys, 
    loading: surveysLoading, 
    currentView, 
    addSurvey, 
    setCurrentView 
  } = useSurveyManager();
  
  const [showVideo, setShowVideo] = useState(false);
  const [step, setStep] = useState<'instructions' | 'surveyInfoForm'>('instructions');

  if (loading || surveysLoading) {
    return (
      <div className="min-h-screen bg-[#FCFAF2] font-[Jost] text-[#000] flex items-center justify-center">
        <div className="text-center">
          <div className="w-8 h-8 border-4 border-[#B3935E] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-gray-600">Loading...</p>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#FCFAF2] font-[Jost] text-[#000] pb-20">
      {/* Login & Modals */}
      {!isAuthenticated && <LoginOverlay />}
      {showVideo && <VideoModal setShowVideoAction={setShowVideo} />}

      {/* Main Content - Conditional based on current view */}
      {currentView === 'dashboard' ? (
        /* Dashboard View */
        <div className="container mx-auto px-4 pt-10 lg:pt-14 max-w-6xl">
          <SurveyDashboard 
            surveys={surveys} 
            onCreateNew={() => setCurrentView('create')} 
          />
        </div>
      ) : (
        /* Survey Creation View - Three Column Grid Layout */
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_320px] gap-0 lg:gap-8 pt-10 lg:pt-14 min-h-[120vh]">
          {/* Left Column - Fixed 200px for logo spacing */}
          <div className="hidden lg:block">
            {/* Back to Dashboard button if user has surveys */}
            {surveys.length > 0 && (
              <div className="pt-8">
                <button
                  onClick={() => setCurrentView('dashboard')}
                  className="text-[#B3935E] hover:text-[#A08549] text-sm font-medium flex items-center space-x-1"
                >
                  <span>←</span>
                  <span>Back to Dashboard</span>
                </button>
              </div>
            )}
          </div>

          {/* Center Column - Fluid content with centered form */}
          <div className="px-4 lg:px-0">
            {/* Mobile back button */}
            {surveys.length > 0 && (
              <div className="mb-4 lg:hidden">
                <button
                  onClick={() => setCurrentView('dashboard')}
                  className="text-[#B3935E] hover:text-[#A08549] text-sm font-medium flex items-center space-x-1"
                >
                  <span>←</span>
                  <span>Back to Dashboard</span>
                </button>
              </div>
            )}
            
            {step === 'instructions' ? (
              <>
                {/* Title - 36-48px with 40-56px top gap */}
                <div className="mb-8 lg:mb-12 text-center">
                  <h2 className="text-3xl lg:text-4xl font-bold mb-2 text-[#2E2F32]">
                    Upload a Survey <span className="font-normal text-sm">(Using a laptop is advised)</span>
                  </h2>
                </div>

                {/* Form Container - Narrower width like original */}
                <div className="max-w-2xl mx-auto">
                  <InstructionsSection setShowVideoAction={setShowVideo} setStepAction={setStep} />
                </div>
              </>
            ) : (
              <SurveyInfoForm 
                setStepAction={(step) => setStep(step)} 
                onSurveyCreatedAction={addSurvey}
              />
            )}
          </div>

          {/* Right Column - Fixed 320px Note box */}
          <div className="px-4 lg:px-0 lg:self-start lg:pt-0 pt-8">
            <div className="bg-[#FFFCF5] p-6 rounded-lg w-full lg:w-[320px]">
              <h3 className="font-bold text-lg mb-2">Note!</h3>
              <p className="text-sm text-gray-700 leading-relaxed">
                If we notice that any of these settings have been tampered with after upload, your survey will be automatically taken down from the platform with no notice. You will not be entitled to any refund for the cost incurred on the survey.
              </p>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
